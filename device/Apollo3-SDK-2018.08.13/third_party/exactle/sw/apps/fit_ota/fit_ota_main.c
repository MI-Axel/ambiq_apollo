...
enum
{
  ...
  FIT_AMOTA_RESET_TIMER_IND,              /*! AMOTA reset timer expired */
  FIT_AMOTA_DISCONNECT_TIMER_IND,         /*! AMOTA disconnect timer expired */
};
  ...
  if (pMsg->ccc.idx == FIT_AMOTAS_TX_CCC_IDX)
  {
    if (pMsg->ccc.value == ATT_CLIENT_CFG_NOTIFY)
    {
      ...
      amotas_start((dmConnId_t) pMsg->ccc.hdr.param, 
        FIT_AMOTA_RESET_TIMER_IND,
        FIT_AMOTA_DISCONNECT_TIMER_IND, 
        FIT_AMOTAS_TX_CCC_IDX);
    }
    else
    {
      // notify disabled
...
static void fitProcMsg(fitMsg_t *pMsg)
{
  ...
  switch(pMsg->hdr.event)
  {
    ...
    case FIT_AMOTA_RESET_TIMER_IND:
    case FIT_AMOTA_DISCONNECT_TIMER_IND:
