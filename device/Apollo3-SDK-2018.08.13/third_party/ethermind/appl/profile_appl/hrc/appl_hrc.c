 ...
 *  \file appl_hrs.c
 *
 *  This is a stand-alone Heart Rate Collector application.
 *
...
/** HRC Characteristic related information */
typedef struct
{
    ...
    ATT_ATTR_HANDLE hrs_msrmt_hdl;

    /* Heart Rate Measurement - CCC */
    ATT_ATTR_HANDLE hrs_msrmt_ccc_hdl;

    /* Heart Rate Sensor Location */
    ATT_ATTR_HANDLE hrs_sensor_loc_hdl;

    /* Heart Rate Control Point */
    ATT_ATTR_HANDLE hrs_cntrl_pnt_hdl;
...
/* Manufacturer Name */
#define DIS_MANUFACTURER_NAME_HDL     (hrc_char_info.dis_manufacturer_name_hdl)

/* Model Number */
#define DIS_MODEL_NUMBER_HDL          (hrc_char_info.dis_model_number_hdl)

/* Serial Number */
#define DIS_SERIAL_NUMBER_HDL         (hrc_char_info.dis_serial_number_hdl)

/* Hardware Revision */
#define DIS_HARDWARE_REVISION_HDL     (hrc_char_info.dis_hardware_revision_hdl)

/* Firmware Revision */
#define DIS_FIRMWARE_REVISION_HDL     (hrc_char_info.dis_firmware_revision_hdl)

/* Software Revision */
#define DIS_SOFTWARE_REVISION_HDL     (hrc_char_info.dis_software_revision_hdl)

/* System Id */
#define DIS_SYSTEM_ID_HDL             (hrc_char_info.dis_sys_id_hdl)

/* RegCertDataList */
#define DIS_REG_CERT_DATA_LIST_HDL    (hrc_char_info.dis_reg_cert_data_list_hdl)

/* HRS - Heart Rate Service */

/* Heart Rate Measurement */
#define HRS_HR_MSRMT_HDL          (hrc_char_info.hrs_msrmt_hdl)

/* Heart Rate Measurement - CCC */
#define HRS_HR_MSRMT_CCC_HDL      (hrc_char_info.hrs_msrmt_ccc_hdl)

/* Heart Rate Sensor Location */
#define HRS_SENSOR_LOC_HDL        (hrc_char_info.hrs_sensor_loc_hdl)

/* Heart Rate Control Point */
#define HRS_CNRTL_PNT_HDL         (hrc_char_info.hrs_cntrl_pnt_hdl)

/* BAS - Battery Service */

/* Battery Level */
#define BAS_BATTERY_LEVEL_HDL          (hrc_char_info.bas_batt_level_hdl)

#define APPL_CLI_CNFG_VAL_LENGTH    2

#define APPL_HRC_INFO_INIT()\
        ...
        HRS_HR_MSRMT_HDL     = ATT_INVALID_ATTR_HANDLE_VAL;\
        HRS_HR_MSRMT_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        HRS_SENSOR_LOC_HDL   = ATT_INVALID_ATTR_HANDLE_VAL;\
        HRS_CNRTL_PNT_HDL    = ATT_INVALID_ATTR_HANDLE_VAL;\
...
DECL_STATIC HRC_CHAR_INFO hrc_char_info; ;
...
#ifdef APPL_MENU_OPS
            CONSOLE_OUT("Command Status Event. Status: 0x%02X\n", status);
#endif /* APPL_MENU_OPS */
    ...
    if(0 < length)
    {
        if(0x01 == type)
        {
            ...
            for(index = 0; index<length; index=index+4)
            {
                ...
                {
                    if (GATT_HR_MSRMT_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        HRS_HR_MSRMT_CCC_HDL = handle;
                    }
                }
            }
        }
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_HR_MSRMT_CHARACTERISTIC:
                HRS_HR_MSRMT_HDL = value_handle;
                break;

            case GATT_BODY_SENSOR_LOC_CHARACTERISTIC:
                HRS_SENSOR_LOC_HDL = value_handle;
                break;

            case GATT_HR_CNTRL_POINT:
                HRS_CNRTL_PNT_HDL = value_handle;
                break;
           ...
           )
{
    ...
    case SMP_LONG_TERM_KEY_REQUEST:
        ...
        smp_unpack_2_byte_param(&ediv, &data[8]);
 ...
 *  \return   : None.
 */
void appl_discover_heart_rate_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_HEART_RATE_SERVICE;
...
void appl_profile_operations (void)
{
    ...
    while (1)
    {
        ...
        case 1:
            break;
        case 10:
            appl_discover_heart_rate_service();
            break;
        case 11:
            ...
            value.val = cfg_val;
            appl_write_req (HRS_HR_MSRMT_CCC_HDL,&value);
            break;
        case 12:
            appl_read_req (HRS_SENSOR_LOC_HDL);
            break;
        case 13:
            cfg_val[0] = 0x01;
            value.len = 0x0001;
            value.val = cfg_val;
            appl_write_req (HRS_CNRTL_PNT_HDL,&value);
            break;
        case 14:
...
void console_app_read (UCHAR * choice, UCHAR * value, UINT16 length)
{
    ...
    switch (*choice)
    {
        ...
        case (0x80 | 2):
            ...
            break;
       case 4:
            CONSOLE_OUT("%s", HRS_menu);
        *choice = 1;
            break;
       case 5:
            CONSOLE_OUT("%s", DIS_menu);
        *choice = 1;
            break;
       case 6:
        ...
        *choice = 1;
        ...
        case 11:
            ...
            appl_write_req (HRS_HR_MSRMT_CCC_HDL,&att_value);
            *choice = 1;
            break;
        case 12:
            appl_read_req (HRS_SENSOR_LOC_HDL);
        ...
        case 13:
            cfg_val[0] = 0x01;
            att_value.len = 0x0001;
            att_value.val = cfg_val;
            appl_write_req (HRS_CNRTL_PNT_HDL,&att_value);
            *choice = 1;
            break;
        case 14:
            cli_cfg = GATT_CLI_CNFG_DEFAULT;
