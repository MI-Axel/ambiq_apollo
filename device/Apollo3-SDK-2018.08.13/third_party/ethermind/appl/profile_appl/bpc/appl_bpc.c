 ...
 *  \file appl_bpc.c
 *
 *  This is a stand-alone Blood Pressure Collector application.
 *
...
/** BPC Characteristic related information */
typedef struct
{
    ...
    ATT_ATTR_HANDLE bps_bp_msrmt_hdl;

    /* Blood Pressure Measurement - CCC */
    ATT_ATTR_HANDLE bps_bp_msrmt_ccc_hdl;

    /* Intermediate Cuff Pressure */
    ATT_ATTR_HANDLE bps_intr_cuff_prsr_hdl;

    /* Intermediate Cuff Pressure - CCC */
    ATT_ATTR_HANDLE bps_intr_cuff_prsr_ccc_hdl;

    /* Blood Pressure Feature */
    ATT_ATTR_HANDLE bps_bp_feature_hdl;
...
/* Manufacturer Name */
#define DIS_MANUFACTURER_NAME_HDL     (bpc_char_info.dis_manufacturer_name_hdl)

/* Model Number */
#define DIS_MODEL_NUMBER_HDL          (bpc_char_info.dis_model_number_hdl)

/* Serial Number */
#define DIS_SERIAL_NUMBER_HDL         (bpc_char_info.dis_serial_number_hdl)

/* Hardware Revision */
#define DIS_HARDWARE_REVISION_HDL     (bpc_char_info.dis_hardware_revision_hdl)

/* Firmware Revision */
#define DIS_FIRMWARE_REVISION_HDL     (bpc_char_info.dis_firmware_revision_hdl)

/* Software Revision */
#define DIS_SOFTWARE_REVISION_HDL     (bpc_char_info.dis_software_revision_hdl)

/* System Id */
#define DIS_SYSTEM_ID_HDL             (bpc_char_info.dis_sys_id_hdl)

/* RegCertDataList */
#define DIS_REG_CERT_DATA_LIST_HDL    (bpc_char_info.dis_reg_cert_data_list_hdl)

/* BPS - Blood Pressure Service */

/* Blood Pressure Measurement */
#define BPS_BP_MSRMT_HDL          (bpc_char_info.bps_bp_msrmt_hdl)

/* Blood Pressure Measurement - CCC */
#define BPS_BP_MSRMT_CCC_HDL      (bpc_char_info.bps_bp_msrmt_ccc_hdl)

/* Intermediate Cuff Pressure */
#define BPS_INTR_CUFF_PRSR_HDL           (bpc_char_info.bps_intr_cuff_prsr_hdl)

/* Intermediate Cuff Pressure - CCC */
#define BPS_INTR_CUFF_PRSR_CCC_HDL       (bpc_char_info.bps_intr_cuff_prsr_ccc_hdl)

/* Blood Pressure Feature */
#define BPS_BP_FEATURE_HDL           (bpc_char_info.bps_bp_feature_hdl)

/* BAS - Battery Service */

/* Battery Level */
#define BAS_BATTERY_LEVEL_HDL          (bpc_char_info.bas_batt_level_hdl)

#define APPL_CLI_CNFG_VAL_LENGTH    2

#define APPL_BPC_INFO_INIT()\
        ...
        BPS_BP_MSRMT_HDL         = ATT_INVALID_ATTR_HANDLE_VAL;\
        BPS_BP_MSRMT_CCC_HDL     = ATT_INVALID_ATTR_HANDLE_VAL;\
        BPS_INTR_CUFF_PRSR_HDL     = ATT_INVALID_ATTR_HANDLE_VAL;\
        BPS_INTR_CUFF_PRSR_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        BPS_BP_FEATURE_HDL       = ATT_INVALID_ATTR_HANDLE_VAL;\
...
DECL_STATIC BPC_CHAR_INFO bpc_char_info; ;
    ...
    if(0 < length)
    {
        if(0x01 == type)
        {
            ...
            for(index = 0; index<length; index=index+4)
            {
                ...
                {
                    if (GATT_BP_MSRMT_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        BPS_BP_MSRMT_CCC_HDL = handle;
                    }
                    else if (GATT_INTRMDT_CUFF_PRSR_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        BPS_INTR_CUFF_PRSR_CCC_HDL = handle;
                    }
                }
            }
        }
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_BP_MSRMT_CHARACTERISTIC:
                BPS_BP_MSRMT_HDL = value_handle;
                break;

            case GATT_INTRMDT_CUFF_PRSR_CHARACTERISTIC:
                BPS_INTR_CUFF_PRSR_HDL = value_handle;
                break;

            case GATT_BP_FEATURE_CHARACTERISTIC:
                BPS_BP_FEATURE_HDL = value_handle;
                break;
 ...
 *  \return   : None.
 */
void appl_discover_blood_pressure_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_BLOOD_PRESSURE_SERVICE;
...
void appl_profile_operations (void)
{
    ...
    while (1)
    {
        ...
        case 1:
            break;
        case 10:
            appl_discover_blood_pressure_service();
            break;
        case 11:
            ...
            value.val = cfg_val;
            appl_write_req (BPS_BP_MSRMT_CCC_HDL,&value);
            break;
        case 12:
        ...
        case 13:
            ...
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req (BPS_INTR_CUFF_PRSR_CCC_HDL,&value);
            break;
        case 14:
            cli_cfg = GATT_CLI_CNFG_DEFAULT;
            BT_PACK_LE_2_BYTE(cfg_val, &cli_cfg);
            ...
            break;
        case 15:
            appl_read_req(BPS_BP_FEATURE_HDL);
            break;
        case 50:
