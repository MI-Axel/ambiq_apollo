 ...
 *  \file appl_pasp.c
 *
 *  This is a stand-alone Phone Alert Status Client application.
 *
...
/** PASP Characteristic related information */
typedef struct
{
    /* PASS - Phone Alert Status Service */

    /* Alert Status */
    ATT_ATTR_HANDLE pass_alert_status_hdl;

    /* Alert Status - CCC */
    ATT_ATTR_HANDLE pass_alert_status_ccc_hdl;

    /* Ringer Setting */
    ATT_ATTR_HANDLE pass_ringer_setting_hdl;

    /* Ringer Setting - CCC */
    ATT_ATTR_HANDLE pass_ringer_setting_ccc_hdl;

    /* Ringer Control Point */
    ATT_ATTR_HANDLE pass_ringer_cntrl_pnt_hdl;
...
/* PASS - Phone Alert Status Service */
/* Alert Status */
#define PASS_ALERT_STATUS_HDL        (pasp_char_info.pass_alert_status_hdl)

/* Alert Status - CCC */
#define PASS_ALERT_STATUS_CCC_HDL    (pasp_char_info.pass_alert_status_ccc_hdl)

/* Ringer Setting */
#define PASS_RINGER_SETTING_HDL      (pasp_char_info.pass_ringer_setting_hdl)

/* Ringer Setting - CCC */
#define PASS_RINGER_SETTING_CCC_HDL  (pasp_char_info.pass_ringer_setting_ccc_hdl)

/* Ringer Control Point */
#define PASS_RINGER_CNTRL_PNT_HDL   (pasp_char_info.pass_ringer_cntrl_pnt_hdl)

/* BAS - Battery Service */

/* Battery Level */
#define BAS_BATTERY_LEVEL_HDL        (pasp_char_info.bas_batt_level_hdl)

#define APPL_CLI_CNFG_VAL_LENGTH    2
#define APPL_PASP_INFO_INIT()\
        PASS_ALERT_STATUS_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        PASS_ALERT_STATUS_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        PASS_RINGER_SETTING_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        PASS_RINGER_SETTING_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        PASS_RINGER_CNTRL_PNT_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        BAS_BATTERY_LEVEL_HDL = ATT_INVALID_ATTR_HANDLE_VAL;

#define SET_SILENT_MODE_CMD           1
#define MUTE_ONCE_CMD                 2
#define CANCEL_SILENT_MODE_CMD        3
...
DECL_STATIC PASP_CHAR_INFO pasp_char_info;
        ...
        if(0x01 == type)
        {
            ...
            for(index = 0; index<length; index=index+4)
            {
                ...
                if (GATT_CLIENT_CONFIG == uuid)
                {
                    ...
                    if (GATT_ALERT_STATUS_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        PASS_ALERT_STATUS_CCC_HDL = handle;
                    }
                    else if (GATT_RINGER_SETTING_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        PASS_RINGER_SETTING_CCC_HDL = handle;
                    }
                }
            }
        }
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_ALERT_STATUS_CHARACTERISTIC:
                PASS_ALERT_STATUS_HDL = value_handle;
                break;

            case GATT_RINGER_SETTING_CHARACTERISTIC:
                PASS_RINGER_SETTING_HDL = value_handle;
                break;

            case GATT_RINGER_CONTROL_POINT_CHARACTERISTIC:
                PASS_RINGER_CNTRL_PNT_HDL = value_handle;
                break;
 ...
 *  \return   : None.
 */
void appl_discover_phone_alert_status_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_PHONE_ALERT_STATUS_SERVICE;
...
void appl_profile_operations (void)
{
    ...
    while (1)
    {
        ...
        switch (choice)
        {
            ...
            case 10:
                appl_discover_phone_alert_status_service();
                break;
            ...
            case 11:
                ...
                value.val = cfg_val;
                appl_write_req (PASS_ALERT_STATUS_CCC_HDL,&value);
                break;
            ...
            case 13:
                appl_read_req (PASS_ALERT_STATUS_HDL);
            ...
            case 14:
                ...
                value.len = APPL_CLI_CNFG_VAL_LENGTH;
                value.val = cfg_val;
                appl_write_req (PASS_RINGER_SETTING_CCC_HDL,&value);
                break;
            ...
            case 16:
                appl_read_req (PASS_RINGER_SETTING_HDL);

            case 17:
                cfg_val[0] = SET_SILENT_MODE_CMD;
                value.len = 1;
                value.val = cfg_val;
                appl_write_cmd (PASS_RINGER_CNTRL_PNT_HDL,&value);
                break;

            case 18:
                cfg_val[0] = CANCEL_SILENT_MODE_CMD;
                value.len = 1;
                value.val = cfg_val;
                appl_write_cmd (PASS_RINGER_CNTRL_PNT_HDL,&value);
                break;
