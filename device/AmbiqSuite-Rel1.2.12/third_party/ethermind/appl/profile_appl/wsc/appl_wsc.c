 ...
 *  \file appl_wsc.c
 *
 *  This is a stand-alone Weight Scale Collector application.
 *
...
/* Scan parameters */
static UINT16  scan_interval;
static UINT16  scan_window;

#define APPL_GAP_SET_SCAN_INTERVAL(data)\
        scan_interval = (data)

#define APPL_GAP_SET_SCAN_WINDOW(data)\
        scan_window = (data)
...
/* Scan Interval */
#define APPL_GAP_GET_SCAN_INTERVAL()           scan_interval

/* Scan Window */
#define APPL_GAP_GET_SCAN_WINDOW()             scan_window
...
void appl_discover_wss_service(void);
void appl_discover_bcs_service(UINT32 choice);
void appl_discover_uds_service(void);
...
void appl_uds_char_operations(void);
...
/** WSC Characteristic related information */
typedef struct
{
    /* WSS - Weight Scale Service */

    /* Weight Scale Feature */
    ATT_ATTR_HANDLE wss_weight_scale_feature_hdl;

    /* Weight Measurement */
    ATT_ATTR_HANDLE wss_weight_msmnt_hdl;

    /* Weight Measurement - CCC */
    ATT_ATTR_HANDLE wss_weight_msmnt_ccc_hdl;
...
typedef struct
{
    ...
    ATT_ATTR_HANDLE bcs_body_comp_feature_hdl;

    /* Body Composition Measurement */
    ATT_ATTR_HANDLE bcs_body_comp_msmnt_hdl;

    /* Body Composition Measurement ccc */
    ATT_ATTR_HANDLE bcs_body_comp_msmnt_ccc_hdl;
...
{
    /* First Name */
    ATT_ATTR_HANDLE uds_first_name_hdl;
    ATT_ATTR_HANDLE uds_last_name_hdl;
    ATT_ATTR_HANDLE uds_email_hdl;
    ATT_ATTR_HANDLE uds_age_hdl;
    ATT_ATTR_HANDLE uds_gender_hdl;
    ATT_ATTR_HANDLE uds_db_change_incremnt_hdl;
    ATT_ATTR_HANDLE uds_db_change_incremnt_ccc_hdl;
    ATT_ATTR_HANDLE uds_user_idx_hdl;
    ATT_ATTR_HANDLE uds_cp_hdl;
    ATT_ATTR_HANDLE uds_cp_ccc_hdl;
}UDS_CHAR_INFO;

/** Service Changed Characteristic related information */
typedef struct
{
    /* GATT */

    /* Service Changed */
    ATT_ATTR_HANDLE gatt_service_changed_hdl;

    /* Service Changed - CCC */
    ATT_ATTR_HANDLE gatt_service_changed_ccc_hdl;

}SERVICE_CHANGED_CHAR_INFO;
...
/* Weight scale feature */
#define WSS_WEIGHT_SCALE_FEATURE_HDL (wsc_char_info.wss_weight_scale_feature_hdl)

/* Weight Scale Measurement */
#define WSS_WEIGHT_SCALE_MSMNT_HDL (wsc_char_info.wss_weight_msmnt_hdl)

/* Weight Scale Measurement - CCC */
#define WSS_WEIGHT_SCALE_MSMNT_CCC_HDL (wsc_char_info.wss_weight_msmnt_ccc_hdl)

/* Body Composition Feature */
#define BCS_BODY_COMP_FEATURE_HDL (bcs_char_info.bcs_body_comp_feature_hdl)

/* Body Composition Measurement*/
#define BCS_BODY_COMP_MSMNT_HDL (bcs_char_info.bcs_body_comp_msmnt_hdl)

/* Body Composition Measurement - CCC */
#define BCS_BODY_COMP_MSMNT_CCC_HDL (bcs_char_info.bcs_body_comp_msmnt_ccc_hdl)

/* User Data First Name */
#define UDS_FIRST_NAME_HDL (uds_char_info.uds_first_name_hdl)

/* User Data Last Name */
#define UDS_LAST_NAME_HDL (uds_char_info.uds_last_name_hdl)

/* User Data Email */
#define UDS_EMAIL_HDL (uds_char_info.uds_email_hdl)

/* User Data Age */
#define UDS_AGE_HDL (uds_char_info.uds_age_hdl)

/* User Data Gender */
#define UDS_GENDER_HDL (uds_char_info.uds_gender_hdl)

/* User Database Increment */
#define UDS_DB_INCREMNT_HDL (uds_char_info.uds_db_change_incremnt_hdl)

/* User Database Increment CCC */
#define UDS_DB_INCREMNT_CCC_HDL (uds_char_info.uds_db_change_incremnt_ccc_hdl)

/* UDS User Index */
#define UDS_USER_INDEX_HDL (uds_char_info.uds_user_idx_hdl)

/* User Data Control point */
#define UDS_CONTROL_POINT_HDL (uds_char_info.uds_cp_hdl)

/* User Data Control point CCCC */
#define UDS_CONTROL_POINT_CCC_HDL (uds_char_info.uds_cp_ccc_hdl)

/* GATT */
/* Service Changed */
#define GATT_SERVICE_CHANGED_HDL (ser_changed_char_info.gatt_service_changed_hdl)

/* Service Changed - CCC */
#define GATT_SERVICE_CHANGED_CCC_HDL (ser_changed_char_info.gatt_service_changed_ccc_hdl)

#define APPL_CLI_CNFG_VAL_LENGTH    2
#define APPL_REGISTER_NEW_USER      0x01
#define APPL_SEND_CONSENT           0x02
#define APPL_DELETE_USER_DATA       0x03

#define APPL_DCI_VALUE_LEN                4
#define APPL_REGISTER_NEW_USER_REQ_LEN    3
#define APPL_SEND_CONSENT_REQ_LEN         4
#define APPL_DELETE_USER_DATA_REQ_LEN     1

#define APPL_WSC_INFO_INIT()\
        WSS_WEIGHT_SCALE_FEATURE_HDL    = ATT_INVALID_ATTR_HANDLE_VAL;\
        WSS_WEIGHT_SCALE_MSMNT_HDL      = ATT_INVALID_ATTR_HANDLE_VAL;\
        WSS_WEIGHT_SCALE_MSMNT_CCC_HDL  = ATT_INVALID_ATTR_HANDLE_VAL;\
        BCS_BODY_COMP_FEATURE_HDL       = ATT_INVALID_ATTR_HANDLE_VAL;\
        BCS_BODY_COMP_MSMNT_HDL         = ATT_INVALID_ATTR_HANDLE_VAL;\
        BCS_BODY_COMP_MSMNT_CCC_HDL     = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_FIRST_NAME_HDL              = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_LAST_NAME_HDL               = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_EMAIL_HDL                   = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_AGE_HDL                     = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_GENDER_HDL                  = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_DB_INCREMNT_HDL             = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_DB_INCREMNT_CCC_HDL         = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_USER_INDEX_HDL              = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_CONTROL_POINT_HDL           = ATT_INVALID_ATTR_HANDLE_VAL;\
        UDS_CONTROL_POINT_CCC_HDL       = ATT_INVALID_ATTR_HANDLE_VAL;\
        GATT_SERVICE_CHANGED_HDL        = ATT_INVALID_ATTR_HANDLE_VAL;\
        GATT_SERVICE_CHANGED_CCC_HDL    = ATT_INVALID_ATTR_HANDLE_VAL;
...
DECL_STATIC WSC_CHAR_INFO               wsc_char_info;
DECL_STATIC BSC_CHAR_INFO               bcs_char_info;
DECL_STATIC UDS_CHAR_INFO               uds_char_info;
DECL_STATIC SERVICE_CHANGED_CHAR_INFO   ser_changed_char_info;
...
static UCHAR wsc_client_menu[] =
"\n\
    0 - Exit\n\
    1 - Refresh\n\n\
\n\
   --- Weight Scale Service ---\n\
   10 - Discover Weight Scale Service\n\
\n\
   11 - Read Weight Scale Measurement CCD\n\
   12 - Configure Weight Scale Measurment for Indication\n\
   13 - Disable Weight Scale Measurment Indication\n\
   14 - Read Weight Scale Feature\n\n\
  --- Body Composition Service ---\n\
   20 - Discover Body Composition Service\n\
   21 - Read Body Composition Measurement CCD\n\
   22 - Configure Body Composition Measurment for Indication\n\
   23 - Disable Body Composition Measurment Indication\n\
   24 - Read Body Composition Feature\n\n\
  --- User Data Service ---\n\
   30 - Discover User Data Service\n\
\n\
   31 - Read/Update UDS Characteristics\n\
   32 - Read Database Change Increment\n\
   33 - Write Database Change Increment\n\
   34 - Read Database Change Increment CCD\n\
   35 - Configure Database Change Increment for Notification\n\
   36 - Disable Database Change Increment Notification\n\
   37 - Read User Index\n\
   38 - Read User Control Point CCD\n\
   39 - Configure User Control Point for Indication\n\
   40 - Disable User Control Point Indication\n\
   41 - Register New User\n\
   42 - Consent\n\
   43 - Delete User Data\n\n\
  --- Service Changed Service ---\n\
   50 - Discover Service Changed Service\n\
   51 - Configure Service Changed for Indication\n\
   52 - Disable Service Changed Indication\n\n\
Your Option?\n\
";

DECL_STATIC UCHAR uds_char_list[] =
"\n\
    1 - First Name\n\
    2 - Last Name\n\
    3 - Email\n\
    4 - Age[1 octet]\n\
    5 - Gender[0 - Male, 1 - Female, 2 - Unspecified]\n\
\n\
Your Option?\n\
";
#endif /* APPL_MENU_OPS */
...
void appl_poweron_stack(void)
{
    ...
    /* Set Scan Interval and Scan Window Values */
    APPL_GAP_SET_SCAN_INTERVAL(0x0040);
    APPL_GAP_SET_SCAN_WINDOW(0x0040);
    ...
    if(0 < length)
    {
        if(0x01 == type)
        {
            ...
            for(index = 0; index<length; index=index+4)
            {
                ...
                {
                    if (GATT_WS_MSRMT_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        WSS_WEIGHT_SCALE_MSMNT_CCC_HDL = handle;
                    }
                    else if (GATT_BC_MSRMT_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        BCS_BODY_COMP_MSMNT_CCC_HDL = handle;
                    }
                    else if (GATT_UD_DB_CHNG_INC_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        UDS_DB_INCREMNT_CCC_HDL = handle;
                    }
                    else if (GATT_UD_USER_CNTRL_PNT_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        UDS_CONTROL_POINT_CCC_HDL = handle;
                    }
                    else if (GATT_SERVICE_CHANGED_CHARACTERISTIC == \
                         APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                     {
                         GATT_SERVICE_CHANGED_CCC_HDL = handle;
                     }
                }
            }
        }
...
#ifdef APPL_MENU_OPS
            /* Populate the Characteristics Handle */
            switch (uuid)
            {
            case GATT_WS_MSRMT_CHARACTERISTIC:
                WSS_WEIGHT_SCALE_MSMNT_HDL = value_handle;
                break;
            case GATT_WS_FEATURE_CHARACTERISTIC:
                WSS_WEIGHT_SCALE_FEATURE_HDL = value_handle;
                break;
            case GATT_BC_MSRMT_CHARACTERISTIC:
                BCS_BODY_COMP_MSMNT_HDL = value_handle;
                break;
            case GATT_BC_FEATURE_CHARACTERISTIC:
                BCS_BODY_COMP_FEATURE_HDL = value_handle;
                break;
            case GATT_UD_FIRST_NAME_CHARACTERISTIC:
                UDS_FIRST_NAME_HDL = value_handle;
                break;
            case GATT_UD_LAST_NAME_CHARACTERISTIC:
                UDS_LAST_NAME_HDL = value_handle;
                break;
            case GATT_UD_EMAIL_CHARACTERISTIC:
                UDS_EMAIL_HDL = value_handle;
                break;
            case GATT_UD_AGE_CHARACTERISTIC:
                UDS_AGE_HDL = value_handle;
                break;
            case GATT_UD_GENDER_CHARACTERISTIC:
                UDS_GENDER_HDL = value_handle;
                break;
            case GATT_UD_DB_CHNG_INC_CHARACTERISTIC:
                UDS_DB_INCREMNT_HDL = value_handle;
                break;
            case GATT_UD_USER_INDEX_CHARACTERISTIC:
                UDS_USER_INDEX_HDL = value_handle;
                break;
            case GATT_UD_USER_CNTRL_PNT_CHARACTERISTIC:
                UDS_CONTROL_POINT_HDL = value_handle;
                break;
            case GATT_SERVICE_CHANGED_CHARACTERISTIC:
                 GATT_SERVICE_CHANGED_HDL = value_handle;
                 break;
            default:
                APPL_TRC ("Unknown\n");
                break;
...
API_RESULT appl_discover_secondary_service
(
    /* IN */ ATT_UUID16          * uuid,
    /* IN */ UCHAR                 uuid_frmt
)
{
    ...
    find_by_type_val_pram.range.start_handle = ATT_ATTR_HANDLE_START_RANGE;
    find_by_type_val_pram.range.end_handle = ATT_ATTR_HANDLE_END_RANGE;
    find_by_type_val_pram.uuid = GATT_SECONDARY_SERVICE;
...
void appl_discover_wss_service(void)
{
    ...
    uuid16 = GATT_WEIGHT_SCALE_SERVICE;
...
void appl_discover_bcs_service(UINT32 choice)
{
    ...
    uuid16 = GATT_BODY_COMPOSITION_SERVICE;

    if (1 == choice)
    {
        /* Discover Body composition as primary Service */
        appl_discover_service
        (
            &uuid16,
            ATT_16_BIT_UUID_FORMAT
    ...
    {
        /* Discover Body composition as secondary Service  */
        appl_discover_secondary_service
        (
            &uuid16,
            ATT_16_BIT_UUID_FORMAT
        );
 ...
 *  \return   : None.
 */
void appl_discover_gatt_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_GATT_SERVICE;
...
{
    int             choice;
    UINT32          appl_uds_char;
    ATT_ATTR_HANDLE appl_uds_handle;
    UCHAR           val_arr[100];
    ATT_VALUE       value;

    CONSOLE_OUT("Read/Write UDS Characteristics\n");
    CONSOLE_OUT("1 (Read), 0 (Write)\n");
    CONSOLE_IN("%u", &choice);

    CONSOLE_OUT("Select the UDS Characteristic\n");
    CONSOLE_OUT("%s", uds_char_list);
    CONSOLE_IN("%d", &appl_uds_char);
    switch (appl_uds_char)
    {
    case 1:
        appl_uds_handle = UDS_FIRST_NAME_HDL;
        break;
    case 2:
        appl_uds_handle = uds_char_info.uds_last_name_hdl;
        break;
    case 3:
        appl_uds_handle = uds_char_info.uds_email_hdl;
        break;
    case 4:
        appl_uds_handle = uds_char_info.uds_age_hdl;
        break;
    case 5:
        appl_uds_handle = uds_char_info.uds_gender_hdl;
        break;
    default:
    ...
    if (0 == choice)
    {
        CONSOLE_OUT("Enter data to be sent: ");
        fflush(stdin);
        fflush(stdout);
        CONSOLE_IN("%s", &val_arr);

        value.val = val_arr;
        value.len = (UINT16)BT_str_len(val_arr);
        appl_write_req(appl_uds_handle, &value);
    }
    else
    {
        appl_read_req(appl_uds_handle);
    }
}
/* ------------------------------- Dummy Functions */
...
void appl_profile_operations (void)
{
    ...
    while (1)
    {
        ...
        case 1:
            break;
        case 10:
            appl_discover_wss_service();
            break;
        case 11:
            appl_read_req(WSS_WEIGHT_SCALE_MSMNT_CCC_HDL);
            break;

        case 12:
            data = GATT_CLI_CNFG_INDICATION;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req(WSS_WEIGHT_SCALE_MSMNT_CCC_HDL, &value);
            break;

        case 13:
            data = GATT_CLI_CNFG_DEFAULT;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req (WSS_WEIGHT_SCALE_MSMNT_CCC_HDL,&value);
        ...
        case 14:
            appl_read_req(WSS_WEIGHT_SCALE_FEATURE_HDL);
            break;

        case 20:
            CONSOLE_OUT("1 - Discover as Primary Service\n" \
                    "2 - Discover as Secondary Service\n");
            CONSOLE_IN("%d", &choice);
            appl_discover_bcs_service(choice);
            break;
        case 21:
            appl_read_req(BCS_BODY_COMP_MSMNT_CCC_HDL);
            break;
        case 22:
            data = GATT_CLI_CNFG_INDICATION;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req(BCS_BODY_COMP_MSMNT_CCC_HDL, &value);
            break;
        case 23:
            data = GATT_CLI_CNFG_DEFAULT;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            ...
            break;
        case 24:
            appl_read_req(BCS_BODY_COMP_FEATURE_HDL);
            break;

        case 30:
            appl_discover_uds_service();
            break;
        case 31:
            appl_uds_char_operations();
            break;
        case 32:
            appl_read_req(UDS_DB_INCREMNT_HDL);
            break;
        case 33:
            CONSOLE_OUT("Please enter the Database Change Increment Value[Hex]: ");
            while (index < APPL_DCI_VALUE_LEN)
            {
                CONSOLE_IN("%x", &data);
                val_arr[index] = (UCHAR)data;
                index++;
            }
            value.val = val_arr;
            value.len = APPL_DCI_VALUE_LEN;
            appl_write_req(UDS_DB_INCREMNT_HDL, &value);
            break;
        case 34:
            appl_read_req(UDS_DB_INCREMNT_CCC_HDL);
            break;
        case 35:
            data = GATT_CLI_CNFG_NOTIFICATION;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req(UDS_DB_INCREMNT_CCC_HDL, &value);
            break;
        case 36:
            data = GATT_CLI_CNFG_DEFAULT;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            ...
            break;
        case 37:
            appl_read_req(UDS_USER_INDEX_HDL);
            break;
        case 38:
            appl_read_req(UDS_CONTROL_POINT_CCC_HDL);
            break;
        case 39:
            data = GATT_CLI_CNFG_INDICATION;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req(UDS_CONTROL_POINT_CCC_HDL, &value);
            break;
        case 40:
            data = GATT_CLI_CNFG_DEFAULT;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            ...
            break;
        case 41:
            val_arr[0] = APPL_REGISTER_NEW_USER;
            CONSOLE_OUT("Please enter the Consent code for new user [Hex]: ");
            CONSOLE_IN("%x", &data);
            BT_PACK_LE_2_BYTE(val_arr + 1, &data);
            value.val = val_arr;
            value.len = APPL_REGISTER_NEW_USER_REQ_LEN;

            appl_write_req(UDS_CONTROL_POINT_HDL, &value);
            break;
        case 42:
            val_arr[0] = APPL_SEND_CONSENT;
            CONSOLE_OUT("Please enter the User Index[Hex]: ");
            CONSOLE_IN("%x", &data);
            val_arr[1] = (UCHAR)data;
            CONSOLE_OUT("Please enter the Consent code [Hex]: ");
            CONSOLE_IN("%x", &data);
            BT_PACK_LE_2_BYTE(val_arr + 2, &data);
            value.val = val_arr;
            value.len = APPL_SEND_CONSENT_REQ_LEN;

            appl_write_req(UDS_CONTROL_POINT_HDL, &value);
            break;
        case 43:
            val_arr[0] = APPL_DELETE_USER_DATA;
            value.val = val_arr;
            value.len = APPL_DELETE_USER_DATA_REQ_LEN;

            appl_write_req(UDS_CONTROL_POINT_HDL, &value);
            break;
        case 50:
            appl_discover_gatt_service ();
            break;
        case 51:
            data = GATT_CLI_CNFG_INDICATION;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req(GATT_SERVICE_CHANGED_CCC_HDL, &value);
            break;
        case 52:
            data = GATT_CLI_CNFG_DEFAULT;
            BT_PACK_LE_2_BYTE(cfg_val, &data);
