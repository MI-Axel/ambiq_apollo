...
void appl_wss_connect(DEVICE_HANDLE  * dq_handle)
{
    ATT_VALUE         value;
    UCHAR             val_arr[2];
    UINT16            cli_cnfg;
    ...
    appl_wss_db_handle.device_id = (*dq_handle);
    appl_bcs_db_handle.device_id = (*dq_handle);
    appl_uds_db_handle.device_id = (*dq_handle);
    appl_dcic_db_handle.device_id = (*dq_handle);

    appl_get_handle_from_device_handle
    (
        appl_wss_db_handle.device_id,
        &appl_handle
    );
...
void appl_profile_menu_handler (void)
{
    ...
    case 10:
        appl_send_wss_measurement(&appl_handle);
        break;

    case 11:
        appl_send_bcs_measurement(&appl_handle);
        break;

    case 12:
        appl_send_dn_chng_measurement(&appl_handle);
        break;
    }
}
