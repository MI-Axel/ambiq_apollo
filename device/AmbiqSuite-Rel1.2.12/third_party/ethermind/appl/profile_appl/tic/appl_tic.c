 ...
 *  \file appl_tic.c
 *
 *  This is a stand-alone Time Client application.
 *
...
/** TIC Characteristic related information */
typedef struct
{
    ...
    ATT_ATTR_HANDLE cts_local_time_info_hdl;

    /* Reference Time Information */
    ATT_ATTR_HANDLE cts_ref_time_info_hdl;

    /* RTUS - Reference Time Update Service */

    /* Time Update Control Point */
    ATT_ATTR_HANDLE rtus_time_update_cp_hdl;

    /* Time Update State */
    ATT_ATTR_HANDLE rtus_time_update_state_hdl;

    /* NDCS - Next DST Change Service */
    /* Time with DST*/
    ATT_ATTR_HANDLE ndcs_time_with_dst;
...
#define APPL_DATETIME_VAL_LENGTH         10
#define APPL_LOCALTIME_VAL_LENGTH        2
...
/* Current Time */
#define CTS_CURRENT_TIME_HDL (tic_char_info.cts_current_time_hdl)

/* Current Time - CCC */
#define CTS_CURRENT_TIME_CCC_HDL (tic_char_info.cts_current_time_ccc_hdl)

/* Local Time Information */
#define CTS_LOCAL_TIME_INFO_HDL (tic_char_info.cts_local_time_info_hdl)

/* Reference Time Information */
#define CTS_REF_TIME_INFO_HDL (tic_char_info.cts_ref_time_info_hdl)

/* RTUS - Reference Time Update Service */

/* Time Update Control Point */
#define RTUS_TIME_UPDATE_CP_HDL (tic_char_info.rtus_time_update_cp_hdl)

/* Time Update State */
#define RTUS_TIME_UPDATE_STATE_HDL (tic_char_info.rtus_time_update_state_hdl)

/* NDCS - Next DST Change Service */

/* Time with DST */
#define NDCS_TIME_WITH_DST (tic_char_info.ndcs_time_with_dst)

/* Battery Level */
#define BAS_BATTERY_LEVEL_HDL (tic_char_info.bas_batt_level_hdl)

#define APPL_TIC_INFO_INIT()\
        CTS_CURRENT_TIME_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        CTS_CURRENT_TIME_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        CTS_LOCAL_TIME_INFO_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        CTS_REF_TIME_INFO_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        RTUS_TIME_UPDATE_CP_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        RTUS_TIME_UPDATE_STATE_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        NDCS_TIME_WITH_DST = ATT_INVALID_ATTR_HANDLE_VAL;\
...
DECL_STATIC TIC_CHAR_INFO tic_char_info;
...
\r\n\
   ...
   16.  Write Current Time \r\n\n\
\r\n\
        -- Local Time Information -- \r\n\
   14.  Read Local Time Information \r\n\
   17.  Write Local Time \r\n\
\r\n\
        -- Reference Time Information -- \r\n\
   15.  Read Reference Time Information \r\n\
\r\n\n\
        ...
        if(0x01 == type)
        {
            ...
            for(index = 0; index<length; index=index+4)
            {
                ...
                if (GATT_CLIENT_CONFIG == uuid)
                {
                    ...
                    if (GATT_CURRENT_TIME_CHARACTERISTIC == APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_LOCAL_TIME_INFO_CHARACTERISTIC:
                CTS_LOCAL_TIME_INFO_HDL = value_handle;
                break;

            case GATT_REF_TIME_INFO_CHARACTERISTIC:
                CTS_REF_TIME_INFO_HDL = value_handle;
                break;

            case GATT_TIME_UPDATE_CONTROL_POINT:
                RTUS_TIME_UPDATE_CP_HDL = value_handle;
                break;

            case GATT_TIME_UPDATE_STATE_CHARACTERISTIC:
                RTUS_TIME_UPDATE_STATE_HDL = value_handle;
                break;

            case GATT_TIME_WITH_DST_CHARACTERISTIC:
                NDCS_TIME_WITH_DST = value_handle;
                break;
 ...
 *  \return   : None.
 */
void appl_discover_reference_time_update_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_REF_TIME_UPDATE_SERVICE;

    /* Discover Reference Time Service */
    appl_discover_service
    (
        &uuid16,
        ATT_16_BIT_UUID_FORMAT
...
/**
 *  Function to initiate Service Discovery for Next DST change.
 *
 *
 *
 *  \return   : None.
 */
void appl_discover_next_dst_change_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_NEXT_DST_CHANGE_SERVICE;

    /* Discover  Next DST change Service */
    appl_discover_service
    (
        &uuid16,
        ATT_16_BIT_UUID_FORMAT
...
void appl_profile_operations (void)
{
    ...
    UCHAR        datetime[APPL_DATETIME_VAL_LENGTH];
    UCHAR        localtime[APPL_LOCALTIME_VAL_LENGTH];
    int          read;
    ...
    while (1)
    {
        ...
        switch (choice)
        {
            ...
            case 16:
                printf("Please enter the Year in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_2_BYTE(&datetime[0], &read);

                printf("Enter Month in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_2_BYTE(&datetime[2], &read);

                printf("Enter Day in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&datetime[3], &read);

                printf("Enter Hours in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&datetime[4], &read);

                printf("Enter Minutes in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&datetime[5], &read);

                printf("Enter Seconds in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&datetime[6], &read);

                printf("Enter Day of the Week in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&datetime[7], &read);

                printf("Enter Fractions256 in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&datetime[8], &read);

                printf("Enter Time adjust reason in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&datetime[9], &read);

                value.len = APPL_DATETIME_VAL_LENGTH;
                value.val = datetime;
                appl_write_req (CTS_CURRENT_TIME_HDL,&value);
                break;

            case 14:
                appl_read_req (CTS_LOCAL_TIME_INFO_HDL);
                break;

            case 17:
                printf("Please enter the Timezone in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&localtime[0], &read);

                printf("Enter DST offset in [DEC]\n");
                scanf("%d", &read);
                BT_PACK_LE_1_BYTE(&localtime[1], &read);

                value.len = APPL_LOCALTIME_VAL_LENGTH;
                value.val = localtime;
                appl_write_req (CTS_LOCAL_TIME_INFO_HDL,&value);
                break;


            case 15:
                appl_read_req (CTS_REF_TIME_INFO_HDL);
                break;

            case 20:
                appl_discover_reference_time_update_service();
                break;
            ...
            case 21:
                ...
                value.len = sizeof(time_update_cp_value);
                value.val = (UCHAR*)&time_update_cp_value;
                appl_write_cmd (RTUS_TIME_UPDATE_CP_HDL,&value);
                break;

            case 22:
                appl_read_req (RTUS_TIME_UPDATE_STATE_HDL);
                break;

            case 30:
                appl_discover_next_dst_change_service();

            case 31:
                appl_read_req (NDCS_TIME_WITH_DST);
