 ...
 *  \file appl_glc.c
 *
 *  This is a stand-alone Glucose Meter Collector application.
 *
...
/** GLC Characteristic related information */
typedef struct
{
    ...
    /* GLS - Glucosemeter Service */

    /* Glucose Measurement */
    ATT_ATTR_HANDLE gls_msrmnt_hdl;

    /* Glucose Measurement - CCC Handle */
    ATT_ATTR_HANDLE gls_msrmnt_ccc_hdl;

    /* Glucose Measurement Context */
    ATT_ATTR_HANDLE gls_msrmnt_cntxt_hdl;

    /* Glucose Measurement Context - CCC Handle */
    ATT_ATTR_HANDLE gls_msrmnt_cntxt_ccc_hdl;

    /* Glucose Feature */
    ATT_ATTR_HANDLE gls_feature_hdl;

    /* Record Access Control Point */
    ATT_ATTR_HANDLE gls_racp_hdl;

    /* Record Access Control Point - CCC Handle */
    ATT_ATTR_HANDLE gls_racp_ccc_hdl;
...
/* Manufacturer Name */
#define DIS_MANUFACTURER_NAME_HDL     (glc_char_info.dis_manufacturer_name_hdl)

/* Model Number */
#define DIS_MODEL_NUMBER_HDL          (glc_char_info.dis_model_number_hdl)

/* Serial Number */
#define DIS_SERIAL_NUMBER_HDL         (glc_char_info.dis_serial_number_hdl)

/* Hardware Revision */
#define DIS_HARDWARE_REVISION_HDL     (glc_char_info.dis_hardware_revision_hdl)

/* Firmware Revision */
#define DIS_FIRMWARE_REVISION_HDL     (glc_char_info.dis_firmware_revision_hdl)

/* Software Revision */
#define DIS_SOFTWARE_REVISION_HDL     (glc_char_info.dis_software_revision_hdl)

/* System Id */
#define DIS_SYSTEM_ID_HDL             (glc_char_info.dis_sys_id_hdl)

/* RegCertDataList */
#define DIS_REG_CERT_DATA_LIST_HDL    (glc_char_info.dis_reg_cert_data_list_hdl)

/* GLS - Glucosemeter Service */

/* Glucose Measurement */
#define GLS_MSRMNT_HDL                (glc_char_info.gls_msrmnt_hdl)

/* Glucose Measurement - CCC Handle */
#define GLS_MSRMNT_CCC_HDL            (glc_char_info.gls_msrmnt_ccc_hdl)

/* Glucose Measurement Context */
#define GLS_MSRMNT_CNTXT_HDL          (glc_char_info.gls_msrmnt_cntxt_hdl)

/* Glucose Measurement Context - CCC Handle */
#define GLS_MSRMNT_CNTXT_CCC_HDL      (glc_char_info.gls_msrmnt_cntxt_ccc_hdl)

/* Glucose Feature */
#define GLS_GLUCOSE_FEATURE_HDL       (glc_char_info.gls_feature_hdl)

/* Record Access Control Point */
#define GLS_RACP_HDL                  (glc_char_info.gls_racp_hdl)

/* Record Access Control Point - CCC Handle */
#define GLS_RACP_CCC_HDL              (glc_char_info.gls_racp_ccc_hdl)

#define APPL_CLI_CNFG_VAL_LENGTH    2
#define APPL_GLC_INFO_INIT()\
        ...
        GLS_MSRMNT_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        GLS_MSRMNT_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        GLS_MSRMNT_CNTXT_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        GLS_MSRMNT_CNTXT_CCC_HDL  = ATT_INVALID_ATTR_HANDLE_VAL;\
        GLS_GLUCOSE_FEATURE_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        GLS_RACP_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        GLS_RACP_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;

#define SEQUENCE_NUMBER_LEN                      2
#define USER_FACING_TIME_LEN                     9

/** RACP Command Length */
#define RACP_NO_OPERAND_LEN                      2 /* (2) */
#define RACP_OPERAND_SEQUENCE_NUMBER_LEN         5 /* (2 + 1 + 2) */
#define RACP_OPERAND_SEQUENCE_NUMBER_PAIR_LEN    7 /* (2 + 1 + 2 + 2) */
#define RACP_OPERAND_USER_FACING_TIME_LEN       12 /* (2 + 1 + 7 + 2) */
#define RACP_OPERAND_USER_FACING_TIME_PAIR_LEN  21 /* (2 + 1 + 7 + 2 + 7 + 2) */
...
#define REPORT_NUM_OF_STORED_RECORDS_OPCODE      4
#define NUM_OF_STORED_RECORDS_RESPONSE_OPCODE    5
#define RESPONSE_CODE_OPCODE                     6

/** RACP Operators */
#define NULL_OPERATOR                            0
#define ALL_RECORDS_OPERATOR                     1
#define LESS_THAN_OR_EQUAL_TO_OPERATOR           2
#define GREATER_THAN_OR_EQUAL_TO_OPERATOR        3
#define WITHIN_RANGE_OF_OPERATOR                 4
#define FIRST_RECORD_OPERATOR                    5
#define LAST_RECORD_OPERATOR                     6

/** RACP Operand Filter Types */
#define SEQUENCE_NUMBER_FILTER_TYPE              1
#define USER_FACING_TIME_FILTER_TYPE             2
...
DECL_STATIC GLC_CHAR_INFO glc_char_info;
...
======== M A I N   M E N U ======== \r\n\
 ...
 \r\n\
   Your Option ? \0";
static UCHAR glc_client_menu[] =" \r\n\
======== C L I E N T   M E N U ======== \r\n\
    0.  Exit \r\n\
    1.  Refresh \r\n\
\r\n\n\
       === Glucosemeter Service ===\r\n\n\
   10.  Discover Glucosemeter Service \r\n\
\r\n\
        -- Glucose Measurement -- \r\n\
   11.  Configure Glucose Measurement \r\n\
   12.  Stop Glucose Measurement Context CCC \r\n\
\r\n\
        -- Glucose Measurement Context -- \r\n\
    ...
    if(0 < length)
    {
        if(0x01 == type)
        {
            ...
            for(index = 0; index<length; index=index+4)
            {
                ...
                {
                    if (GATT_GLUCOSE_MSRMNT_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        GLS_MSRMNT_CCC_HDL = handle;
                    }
                    else if (GATT_GL_MSRMT_CTX_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        GLS_MSRMNT_CNTXT_CCC_HDL = handle;
                    }
                    else if (GATT_RACP_CHARACTERISTIC == \
                        APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        GLS_RACP_CCC_HDL = handle;
                    }
                }
            }
        }
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_GLUCOSE_MSRMNT_CHARACTERISTIC:
                GLS_MSRMNT_HDL = value_handle;
                break;

            case GATT_GL_MSRMT_CTX_CHARACTERISTIC:
                GLS_MSRMNT_CNTXT_HDL = value_handle;
                break;

            case GATT_GL_FEATURE_CHARACTERISTIC:
                GLS_GLUCOSE_FEATURE_HDL = value_handle;
                break;

            case GATT_RACP_CHARACTERISTIC:
                GLS_RACP_HDL = value_handle;
                break;
 ...
 *  \return   : None.
 */
void appl_discover_glucose_meter_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_GLUCOSE_SERVICE;

    /* Discover Glucose Meter Service */
    appl_discover_service
    (
        &uuid16,
        ATT_16_BIT_UUID_FORMAT
      ...
      )
{
    ...
    printf (" 1. Sequence Number [UINT16)\n");
    printf (" 2. User Facing Time (Base Time + Time Offset)\n");
    scanf ("%d", &read);
    if ((1 > read) || (2 < read))
    {
    ...
    (*filter_type) = (UCHAR) read;

    if (SEQUENCE_NUMBER_FILTER_TYPE == (*filter_type))
    {
        printf ("Enter Sequence Number %s:\n",(is_pair)?"Pair":"");
        for (i = 0; i < ((is_pair + 1) * SEQUENCE_NUMBER_LEN); i+=SEQUENCE_NUMBER_LEN)
        {
            scanf("%d", &read);
            BT_PACK_LE_2_BYTE_VAL(operand+i,(UINT16)read);
        }
        return 1;
    }
    else if (USER_FACING_TIME_FILTER_TYPE == (*filter_type))
    {
        printf ("Enter User Facing Time %s:\n",(is_pair)?"Pair":"");
        for (i = 0; i < ((is_pair + 1) * USER_FACING_TIME_LEN); i+=USER_FACING_TIME_LEN)
        {
            printf ("Year : ");
            scanf("%d", &read);
            BT_PACK_LE_2_BYTE_VAL(operand+i,(UINT16)read);
            printf ("Month : ");
            scanf("%d", &read);
            operand[i+2] = (UCHAR) read;
            printf ("Day : ");
            scanf("%d", &read);
            operand[i+3] = (UCHAR) read;
            printf ("Hour : ");
            scanf("%d", &read);
            operand[i+4] = (UCHAR) read;
            printf ("Minute : ");
            scanf("%d", &read);
            operand[i+5] = (UCHAR) read;
            printf ("Second : ");
            scanf("%d", &read);
            operand[i+6] = (UCHAR) read;
            printf ("Offset : ");
            scanf("%d", &read);
            BT_PACK_LE_2_BYTE_VAL(operand+i+7,(INT16)read);
        }
        return 1;
    }
...
UCHAR appl_handle_racp_opcode (UCHAR opcode, UCHAR **value)
{
    ...
    if (ABORT_OPERATION_OPCODE != opcode)
    {
        ...
        printf ("%s", glc_racp_operator_menu);
        scanf ("%d", &read);
        oprtr = (UCHAR) read;
    }
    ...
    switch (opcode)
    {
        ...
        case REPORT_NUM_OF_STORED_RECORDS_OPCODE: /* Fall Through */
            switch (oprtr)
            {
                ...
                case LESS_THAN_OR_EQUAL_TO_OPERATOR: /* Fall Through */
                    (*value)[0] = opcode;
                    (*value)[1] = oprtr;
                    if (appl_read_racp_command_operand
                         (
                           0,
                           &filter_type,
                           operand
                         ))
                    {
                        if (SEQUENCE_NUMBER_FILTER_TYPE == filter_type)
                        {
                            (*value)[2] = SEQUENCE_NUMBER_FILTER_TYPE;
                            BT_mem_copy
                            (
                               (*value)+3,
                               operand,
                        ...
                        {
                            (*value)[2] = USER_FACING_TIME_FILTER_TYPE;
                            BT_mem_copy
                            (
                               (*value)+3,
                               operand,
                            ...
                            /* If the Offset value is 0 then we need not send it's value */
                            if (0 == offset)
                            {
                                length = RACP_OPERAND_USER_FACING_TIME_LEN - 2;
                            }
                            else
                            {
...
void appl_profile_operations (void)
{
    ...
    while (1)
    {
        ...
        switch (choice)
        {
            ...
            case 10:
                appl_discover_glucose_meter_service();
                break;
            ...
            case 11:
                ...
                value.val = cfg_val;
                appl_write_req (GLS_MSRMNT_CCC_HDL,&value);
                break;
            ...
            case 13:
                ...
                value.len = APPL_CLI_CNFG_VAL_LENGTH;
                value.val = cfg_val;
                appl_write_req (GLS_MSRMNT_CNTXT_CCC_HDL,&value);
                break;
            ...
            case 15:
                appl_read_req(GLS_GLUCOSE_FEATURE_HDL);
                break;
            ...
            case 16:
                ...
                value.len = APPL_CLI_CNFG_VAL_LENGTH;
                value.val = cfg_val;
                appl_write_req (GLS_RACP_CCC_HDL,&value);
                break;
            ...
            case 18:
                ...
                value.len = appl_handle_racp_opcode
                            (
                ...
                if (0 != value.len)
                {
                   appl_write_req (GLS_RACP_HDL,&value);
                }
                break;
