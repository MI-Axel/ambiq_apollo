 ...
 *  \file appl_rscc.c
 *
 *  This is a stand-alone Runners Speed & Cadence Collector application.
 *
...
/** RSCC Characteristic related information */
typedef struct
{
    ...
    /* RSCS - Runnig Speed and Cadence Service */

    /* RSC Measurement */
    ATT_ATTR_HANDLE rscs_rsc_msrmnt_hdl;

    /* RSC Measurement - CCC Handle */
    ATT_ATTR_HANDLE rscs_rsc_msrmnt_ccc_hdl;

    /* RSC Feature */
    ATT_ATTR_HANDLE rscs_rsc_feature_hdl;

    /* Sensor Location */
    ATT_ATTR_HANDLE rscs_snsr_loc_hdl;

    /* Speed and Cadence Control Point */
    ATT_ATTR_HANDLE rscs_sc_cp_hdl;

    /* Speed and Cadence Control Point - CCC Handle */
    ATT_ATTR_HANDLE rscs_sc_cp_ccc_hdl;
...
/* Manufacturer Name */
#define DIS_MANUFACTURER_NAME_HDL     (rscc_char_info.dis_manufacturer_name_hdl)

/* Model Number */
#define DIS_MODEL_NUMBER_HDL          (rscc_char_info.dis_model_number_hdl)

/* Serial Number */
#define DIS_SERIAL_NUMBER_HDL         (rscc_char_info.dis_serial_number_hdl)

/* Hardware Revision */
#define DIS_HARDWARE_REVISION_HDL     (rscc_char_info.dis_hardware_revision_hdl)

/* Firmware Revision */
#define DIS_FIRMWARE_REVISION_HDL     (rscc_char_info.dis_firmware_revision_hdl)

/* Software Revision */
#define DIS_SOFTWARE_REVISION_HDL     (rscc_char_info.dis_software_revision_hdl)

/* System Id */
#define DIS_SYSTEM_ID_HDL             (rscc_char_info.dis_sys_id_hdl)

/* RegCertDataList */
#define DIS_REG_CERT_DATA_LIST_HDL    (rscc_char_info.dis_reg_cert_data_list_hdl)

/* RSCS - Running Speed and Cadence Service */

/* RSC Measurement */
#define RSCS_RSC_MSRMNT_HDL           (rscc_char_info.rscs_rsc_msrmnt_hdl)

/* RSC Measurement - CCC Handle */
#define RSCS_RSC_MSRMNT_CCC_HDL       (rscc_char_info.rscs_rsc_msrmnt_ccc_hdl)

/* RSC Feature */
#define RSCS_RSC_FEATURE_HDL          (rscc_char_info.rscs_rsc_feature_hdl)

/* Sensor Location */
#define RSCS_SNSR_LOC_HDL             (rscc_char_info.rscs_snsr_loc_hdl)

/* Speed and Cadence Control Point */
#define RSCS_SC_CP_HDL                (rscc_char_info.rscs_sc_cp_hdl)

/* Record Access Control Point - CCC Handle */
#define RSCS_SC_CP_CCC_HDL            (rscc_char_info.rscs_sc_cp_ccc_hdl)

/* BAS - Battery Service */

/* Battery Level */
#define BAS_BATTERY_LEVEL_HDL        (rscc_char_info.bas_batt_level_hdl)

#define APPL_CLI_CNFG_VAL_LENGTH    2

#define APPL_RSCC_INFO_INIT()\
        ...
        RSCS_RSC_MSRMNT_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        RSCS_RSC_MSRMNT_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        RSCS_RSC_FEATURE_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        RSCS_SNSR_LOC_HDL  = ATT_INVALID_ATTR_HANDLE_VAL;\
        RSCS_SC_CP_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        RSCS_SC_CP_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;
...
DECL_STATIC RSCC_CHAR_INFO rscc_char_info;
...
\r\n\
        -- RSC Measurement -- \r\n\
   11.  Configure RSC Measurement for Notification \r\n\
   12.  Stop RSC Measurement Notification \r\n\
\r\n\
        -- RSC Feature -- \r\n\
   13.  Read RSC Feature \r\n\
\r\n\
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_RSC_MSRMT_CHARACTERISTIC:
                RSCS_RSC_MSRMNT_HDL = value_handle;
                break;

            case GATT_RSC_FEATURE_CHARACTERISTIC:
                RSCS_RSC_FEATURE_HDL = value_handle;
                break;

            case GATT_SENSOR_LOCATION_CHARACTERISTIC:
                RSCS_SNSR_LOC_HDL = value_handle;
                break;

            case GATT_SC_CONTROL_POINT_CHARACTERISTIC:
                RSCS_SC_CP_HDL = value_handle;
                break;
 ...
 *  \return   : None.
 */
void appl_discover_rscs_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_RUNNING_SPEED_AND_CADENCE_SERVICE;

    /* Discover Runner's Speed & Cadence Service */
    appl_discover_service
    (
        &uuid16,
        ATT_16_BIT_UUID_FORMAT
    ...
    while (1)
    {
        ...
        switch (choice)
        {
            ...
            case 11:
                ...
                value.val = cfg_val;
                appl_write_req (RSCS_RSC_MSRMNT_CCC_HDL,&value);
                break;
            ...
            case 13:
                appl_read_req(RSCS_RSC_FEATURE_HDL);
                break;

            case 14:
                appl_read_req(RSCS_SNSR_LOC_HDL);
                break;
            ...
            case 15:
                ...
                value.len = APPL_CLI_CNFG_VAL_LENGTH;
                value.val = cfg_val;
                appl_write_req (RSCS_SC_CP_CCC_HDL,&value);
                break;
            ...
            case 17:
                ...
                CONSOLE_OUT("Enter the Cummulative Value to be set[Hex]:\n");
                ...
                if (0 != value.len)
                {
                    appl_write_req (RSCS_SC_CP_HDL,&value);
                }
                break;
