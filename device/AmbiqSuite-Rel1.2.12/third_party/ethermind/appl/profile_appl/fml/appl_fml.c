 ...
 *  \file appl_fml.c
 *
 *  This is a stand-alone Find Me Locator sample application.
 *
...
/* Alert Level */
#define IAS_ALERT_LEVEL_HDL (fml_char_info.ias_alert_level_hdl)

/* Battery Level */
#define BAS_BATTERY_LEVEL_HDL (fml_char_info.bas_batt_level_hdl)

#define APPL_FML_INFO_INIT()\
...
DECL_STATIC FML_CHAR_INFO fml_char_info;
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_ALERT_LEVEL_CHARACTERISTIC:
                if (GATT_IMMEDIATE_ALERT_SERVICE == APPL_CURRENT_SERVICE_UUID())
                {
