 ...
 *  \file appl_htc.c
 *
 *  This is a stand-alone Health Thermometer Collector application.
 *
...
/** HTC Characteristic related information */
typedef struct
{
    ...
    /* HTS - Heart Thermometer Service */

    /* Temperature Measurement */
    ATT_ATTR_HANDLE hts_temp_msrmt_hdl;

    /* Temperature Measurement - CCC */
    ATT_ATTR_HANDLE hts_temp_msrmt_ccc_hdl;

    /* Intermediate Temperature */
    ATT_ATTR_HANDLE hts_intr_temp_hdl;

    /* Intermediate Temperature - CCC */
    ATT_ATTR_HANDLE hts_intr_temp_ccc_hdl;

    /* Temperature Type */
    ATT_ATTR_HANDLE hts_temp_type_hdl;

    /* Measurement Interval */
    ATT_ATTR_HANDLE hts_msrmt_intrvl_hdl;

    /* Measurement Interval - CCC */
    ATT_ATTR_HANDLE hts_msrmt_intrvl_ccc_hdl;

    /* Measurement Interval - Valid Range Descriptor */
    ATT_ATTR_HANDLE hts_msrmt_intrvl_vrd_hdl;
...
/* Manufacturer Name */
#define DIS_MANUFACTURER_NAME_HDL     (htc_char_info.dis_manufacturer_name_hdl)

/* Model Number */
#define DIS_MODEL_NUMBER_HDL          (htc_char_info.dis_model_number_hdl)

/* Serial Number */
#define DIS_SERIAL_NUMBER_HDL         (htc_char_info.dis_serial_number_hdl)

/* Hardware Revision */
#define DIS_HARDWARE_REVISION_HDL     (htc_char_info.dis_hardware_revision_hdl)

/* Firmware Revision */
#define DIS_FIRMWARE_REVISION_HDL     (htc_char_info.dis_firmware_revision_hdl)

/* Software Revision */
#define DIS_SOFTWARE_REVISION_HDL     (htc_char_info.dis_software_revision_hdl)

/* System Id */
#define DIS_SYSTEM_ID_HDL             (htc_char_info.dis_sys_id_hdl)

/* RegCertDataList */
#define DIS_REG_CERT_DATA_LIST_HDL    (htc_char_info.dis_reg_cert_data_list_hdl)


/* HTS - Health Thermometer Service */

/* Temperature Measurement */
#define HTS_TEMP_MSRMT_HDL          (htc_char_info.hts_temp_msrmt_hdl)

/* Temperature Measurement - CCC */
#define HTS_TEMP_MSRMT_CCC_HDL      (htc_char_info.hts_temp_msrmt_ccc_hdl)

/* Intermediate Temperature */
#define HTS_INTR_TEMP_HDL           (htc_char_info.hts_intr_temp_hdl)

/* Intermediate Temperature - CCC */
#define HTS_INTR_TEMP_CCC_HDL       (htc_char_info.hts_intr_temp_ccc_hdl)

/* Temperature Type */
#define HTS_TEMP_TYPE_HDL           (htc_char_info.hts_temp_type_hdl)

/* Measurement Interval */
#define HTS_MSRMT_INTRVL_HDL        (htc_char_info.hts_msrmt_intrvl_hdl)

/* Measurement Interval - CCC */
#define HTS_MSRMT_INTRVL_CCC_HDL    (htc_char_info.hts_msrmt_intrvl_ccc_hdl)

/* Measurement Interval - Valid Range Descriptor */
#define HTS_MSRMT_INTRVL_VRD_HDL    (htc_char_info.hts_msrmt_intrvl_vrd_hdl)

/* BAS - Battery Service */

/* Battery Level */
#define BAS_BATTERY_LEVEL_HDL          (htc_char_info.bas_batt_level_hdl)

#define APPL_CLI_CNFG_VAL_LENGTH    2

#define APPL_MSRMT_INTRVL_VALID_RANGE_DESC  0x2906

#define APPL_HTC_INFO_INIT()\
        ...
        HTS_TEMP_MSRMT_HDL     = ATT_INVALID_ATTR_HANDLE_VAL;\
        HTS_TEMP_MSRMT_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        HTS_INTR_TEMP_HDL      = ATT_INVALID_ATTR_HANDLE_VAL;\
        HTS_INTR_TEMP_CCC_HDL  = ATT_INVALID_ATTR_HANDLE_VAL;\
        HTS_TEMP_TYPE_HDL      = ATT_INVALID_ATTR_HANDLE_VAL;\
        HTS_MSRMT_INTRVL_HDL   = ATT_INVALID_ATTR_HANDLE_VAL;\
        HTS_MSRMT_INTRVL_CCC_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
        HTS_MSRMT_INTRVL_VRD_HDL = ATT_INVALID_ATTR_HANDLE_VAL;\
...
DECL_STATIC HTC_CHAR_INFO htc_char_info; ;
    ...
    if(0 < length)
    {
        if(0x01 == type)
        {
            ...
            for(index = 0; index<length; index=index+4)
            {
                ...
                {
                    if (GATT_TEMPERATURE_MSMNT_CHARACTERISTIC == APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        HTS_TEMP_MSRMT_CCC_HDL = handle;
                    }
                    else if (GATT_INTERMEDIATE_TEMP_CHARACTERISTIC == APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        HTS_INTR_TEMP_CCC_HDL = handle;
                    }
                    else if (GATT_MSMNT_INTERVAL_CHARATACTERISTIC == APPL_CHAR_GET_UUID(APPL_GET_CURRENT_CHAR_INDEX()))
                    {
                        HTS_MSRMT_INTRVL_CCC_HDL = handle;
                    }
                }
                else if (APPL_MSRMT_INTRVL_VALID_RANGE_DESC == uuid)
                {
                    HTS_MSRMT_INTRVL_VRD_HDL = handle;
                }
            }
        }
        else if(0x02 == type)
    ...
    for (index = 0; index < (unsigned)(data_len/(data_elem_size)); index++ )
    {
        ...
        if (APPL_GD_IN_CHAR_DSCVRY_STATE == APPL_GET_CURRENT_DSCVRY_STATE())
        {
            ...
            case GATT_TEMPERATURE_MSMNT_CHARACTERISTIC:
                HTS_TEMP_MSRMT_HDL = value_handle;
                break;

            case GATT_INTERMEDIATE_TEMP_CHARACTERISTIC:
                HTS_INTR_TEMP_HDL = value_handle;
                break;

            case GATT_TEMPERATURE_TYPE_CHARACTERISTIC:
                HTS_TEMP_TYPE_HDL = value_handle;
                break;

            case GATT_MSMNT_INTERVAL_CHARATACTERISTIC:
                HTS_MSRMT_INTRVL_HDL = value_handle;
                break;
 ...
 *  \return   : None.
 */
void appl_discover_health_thermometer_service(void)
{
    ATT_UUID16             uuid16;

    uuid16 = GATT_HEALTH_THERMOMETER_SERVICE;
...
void appl_profile_operations (void)
{
    ...
    while (1)
    {
        ...
        case 1:
            break;
        case 10:
            appl_discover_health_thermometer_service();
            break;
        case 11:
            ...
            value.val = cfg_val;
            appl_write_req (HTS_TEMP_MSRMT_CCC_HDL,&value);
            break;
        case 12:
            ...
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req (HTS_INTR_TEMP_CCC_HDL,&value);
            break;
        case 13:
            cli_cfg = GATT_CLI_CNFG_INDICATION;
            BT_PACK_LE_2_BYTE(cfg_val, &cli_cfg);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req (HTS_MSRMT_INTRVL_CCC_HDL,&value);
            break;
        case 14:
            appl_read_req(HTS_TEMP_TYPE_HDL);
            break;
        case 15:
            appl_read_req(HTS_MSRMT_INTRVL_HDL);
            break;
        case 16:
            CONSOLE_OUT ("Please enter the new measurement interval[Dec]: ");
            CONSOLE_IN ("%d", &msrmt_intrvl);
            BT_PACK_LE_2_BYTE(cfg_val, &msrmt_intrvl);
            value.len = APPL_CLI_CNFG_VAL_LENGTH;
            value.val = cfg_val;
            appl_write_req (HTS_MSRMT_INTRVL_HDL,&value);
            break;
        case 17:
            appl_read_req(HTS_MSRMT_INTRVL_VRD_HDL);
            break;
        case 18:
