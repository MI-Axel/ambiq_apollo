...
//
// Notice:
//      the difference between nested_pop and pop is: nested_pop will move the 
//      sub buffer head and tail to the head of parent buffer if sub buffer is empty.
//
//*****************************************************************************
void am_audio_buffer_nested_pop(enum_audio_buffer_t buffer_pop_from, enum_audio_buffer_t buffer_dest, void *pvDest, uint32_t ui32Bytes)
{
    bool bReturnVal = false;
//    AM_CRITICAL_BEGIN_VOS;
    am_audio_buffer_index_set(buffer_dest, am_audio_ring_buffers[buffer_pop_from].ui32BufferHead_read);
