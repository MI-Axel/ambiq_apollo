function beamformer = prepare_beamformer_2_struct(handles, mic_array)

METHOD = handles.method.String{handles.method_choice_2.Value};
C = str2double(handles.speed_sound.String);
FFT_LEN = str2double(handles.fft_length_2.String);
ANGLE_TAR = str2double(handles.angle_tar_2.String);
SIGNAL_BEAM_WIDTH = str2double(handles.signal_beamwidth_2.String);
...
if strcmp( METHOD, 'MAXSNR' )
    if handles.arbitrary_angles_checkbox_2.Value
        try
            beamformer.angles = eval(handles.num_angles_2.String);
        catch err
            rethrow(err)
        end
        beamformer.num_angles = length(beamformer.angles);
        beamformer.arbitrary_angles = handles.num_angles_2.String;
    else
        beamformer.num_angles = str2double(handles.num_angles_2.String);
...
elseif strcmp( METHOD, 'CVX' )
    ...
    if handles.arbitrary_freq_checkbox_2.Value
        try
            beamformer.freqs_opt = eval(handles.num_freqs_2.String);
        catch err
            rethrow(err)
        end
        beamformer.num_freqs = length(beamformer.freqs_opt);
        beamformer.arbitrary_freqs = handles.num_freqs_2.String;
    else
        beamformer.num_freqs = str2double(handles.num_freqs_2.String);
        beamformer.freqs_opt = linspace(0, mic_array.samp_freq/2, beamformer.num_freqs);
        beamformer.arbitrary_freqs = 0;
    end
    
    beamformer.min_beamwidth = handles.min_beamwidth_2.Value;
    beamformer.wng = 10^(str2double(handles.white_noise_gain_2.String)/20);
    if beamformer.min_beamwidth
        beamformer.transition_width = str2double(handles.transition_width_2.String);
        if str2double(handles.passband_ripple_2.String) < 0
            msgbox('Passband ripple must be non-negative!')
            error('Passband ripple must be non-negative!')
        else
            beamformer.pass_ripple = 1-10^(str2double(handles.passband_ripple_2.String)/-20);
        end
    end
